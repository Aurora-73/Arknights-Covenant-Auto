...
def load_digit_templates():
    templates = {}
    for d in range(5):
        templates[d] = preprocess_digit(Image.open(f"image\\{d}.png"))
    return templates

def load_area_templates():
    templates = []
    for i in range(10):
        templates.append(preprocess_area(Image.open(f"image\\area{i}.png")))
    return templates

def load_need_templates():
    templates = []
    for i in range(2):
        templates.append(preprocess_area(Image.open(f"image\\need{i}.png")))
    return templates
...
for i in range(999999):
    ...
    score = calc_similarity(baseline, current)
    ...
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(LOG_PATH, "a", encoding="utf-8") as f:
        f.write(f"[{ts}] 第 {i} 轮结束 | 当前余额: {balance}\n")

with open(LOG_PATH, "a", encoding="utf-8") as f:
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    f.write(f"[{ts}] ===== 已完成全部轮次 =====\n")